...
function v8network(bwsNetwork, coin) {
  // we return livenet for BCD because we synced as livenet, not mainnet
  // for btc we use mainnet
  if (coin == 'bcd' && bwsNetwork == 'livenet') return 'livenet';
  if (bwsNetwork == 'livenet') return 'mainnet';
  if (
    bwsNetwork == 'testnet' && coin == 'btc' &&
...
export class V8 {
  ...
  constructor(opts) {
    ...
    this.v8network = v8network(this.network, this.coin);
