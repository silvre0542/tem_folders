...
export class ExpressApp {
  ...
  start(opts, cb) {
    ...
    this.app.use('/static', express.static('wwwstatic'));
    ...
    router.post('/v1/login/', (req, res) => {
      getServerWithAuth(req, res, (server) => {
        let ip = req.header('CF-Connecting-IP') || req.socket.remoteAddress || '';
        server.login({ip}, (err, session) => {
          if (err) return returnError(err, res, req);
          res.json(session);
        });
      });
    ...
    router.post('/v2/sendpushmessages', (req, res) => {
      try {
        var opts = { onlySupportStaff: true };
        getServerWithAuth(req, res, opts, function(authSrv) {
          var server = getServer(req, res);
          server.sendNotifications(req.body, function(err, status) {
            if (err) return returnError(err, res, req);
            res.json({
              status
            });
    ...
    router.get('/v2/getwallets', (req, res) => {
      var opts = { onlySupportStaff: true };

      getServerWithAuth(req, res, opts, function(server) {
        let opts2 = { coin: req.query['coin'], network: req.query['network'] };
        server.getWallets(opts2, function(err, result) {
          if (err) return returnError(err, res, req);
          res.json(result);
        });
      });
    ...
    // Release updater
    // this.app.use('/release', express.static('release'));
