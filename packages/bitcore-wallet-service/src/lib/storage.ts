...
export class Storage {
  ...
  fetchWallets(opts, cb) {
    if (!this.db) return cb('not ready');
    opts = opts || {};
    let coin = (opts.coin || 'bcd').toLowerCase();
    let network = opts.network || 'livenet';

    this.db.collection(collections.WALLETS).aggregate(
      [
        {
          $match: {
            coin,
        ...
        },
        {
          $lookup: {
            from: 'sessions',
            localField: 'copayers.id',
            foreignField: 'copayerId',
            as: 'session'
          }
        },
        {
