...
export class WalletService {
        ...
        (next) => {
          if (!session || !session.isValid()) {
            session = Session.create({
              ...
              os: Utils.extractOSName(this.userAgent)
            });
          } else {
            session.touch();
            session.ip = opts.ip;
            session.userAgent = this.userAgent;
            session.os = Utils.extractOSName(this.userAgent);
          }
          next();
        },
        (next) => {
  ...
  getStatus(opts, cb) {
    ...
    async.parallel(
      ...
      (err) => {
        ...
        // todo FIX THIS! status.balance.XXX
        let w = { id: status.wallet.id, balance: status.balance };
        this.storage.updateWalletBalance(w, (err) => {
          if (err) return cb(err);
          return cb(null, status);
        });
      }
          ...
          checkTxpAlreadyExists(opts.txProposalId, (err, txp) => {
            ...
            async.series(
              [
                ...
                (next) => {
                  if (txp.coin == 'bcd') {
                    this._getBlockchainHeight(wallet.coin, wallet.network, function(err, height, hash) {
                      if (err) return next(err);
                      txp.presentblockhash = hash;
                      return next();
                    });
                  } else
                    return next();
    ...
    this.getWallet({}, (err, wallet) => {
      ...
      async.map(
        ...
        (err, res) => {
          ...
          let minId = parseInt(_.padStart(ts.toString(), 14, '0') + _.repeat('0', 4));
          notifications = _.filter(notifications, x => x.id > minId || x.type == 'CustomMessage');
  ...
  sendNotifications(opts, cb) {
    opts = opts || {};
    if (!opts.wallets)
      return cb(new ClientError('BAD_REQUEST', 'No Wallets in Request'));

    _.forEach(opts.wallets, x => {
      var data = {
        walletId: x,
        message: opts.message,
      ...
      this._notify('CustomMessage', data, {});
      });
    return cb(null, 'OK');
  }
  ...
  getWallets(opts, cb) {
    opts = opts || {};
    this.storage.fetchWallets(opts, function(err, wallets) {
      if (err) return cb(err);
