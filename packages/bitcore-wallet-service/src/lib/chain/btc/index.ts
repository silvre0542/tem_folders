  ...
  getWalletSendMaxInfo(server, wallet, opts, cb) {
    server._getUtxosForCurrentWallet({}, (err, utxos) => {
      ...
      server._getFeePerKb(wallet, opts, (err, feePerKb) => {
        ...
        let amount = _.sumBy(txp.inputs, 'satoshis') - fee;

        // BCD HACK!
        // this is a temporary fix for incorrect BCD fee (it goes to network as x10) * 10
        if (opts.feeMultiplier)
          amount = _.sumBy(txp.inputs, 'satoshis') - fee * opts.feeMultiplier;
