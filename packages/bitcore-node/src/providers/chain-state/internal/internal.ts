...
export class InternalStateProvider implements CSP.IChainStateService {
  ...
  streamBlocksWithTransform(params: CSP.StreamBlocksParams, transform: (data: MongoBound<IBtcBlock>) => IBtcBlock) {
    const { req, res } = params;
    const { query, options } = this.getBlocksQuery(params);

    Storage.apiStreamingFind(BitcoinBlockStorage, query, options, req, res, b => {
      let b2 = transform(b as IBtcBlock);
      return BitcoinBlockStorage._apiTransform(b2);
    });
  }
  ...
  async streamTransactionsWithTransform(
    params: CSP.StreamTransactionsParams,
    transform?: (data: MongoBound<ITransaction>) => ITransaction
  ) {
    ...
    return Storage.apiStreamingFind(TransactionStorage, query, args, req, res, t => {
      ...
      if (transform) transform(t as ITransaction);
  ...
  async streamWalletTransactions(params: CSP.StreamWalletTransactionsParams) {
    ...
    const transactionStream = TransactionStorage.collection
      ...
      .addCursorFlag('noCursorTimeout', true);
    const listTransactionsStream = new ListTransactionsStream(wallet, { chain });
  ...
  async getTopWallets({ chain, network, args }: { chain: string; network: string; args: any }) {
    let { limit = 100, multiplier = 1, minBalance = 0 } = args;
    let divider = 100000000 / multiplier;
    limit = Number(limit);
    minBalance = Number(minBalance);
    const results = await CoinStorage.collection
      .aggregate<{
        wallet: string;
      ...
      }>([
        {
          $match: {
            ...
            wallets: {
              $gt: new ObjectId('000000000000000000000000')
            },
            spentHeight: {
              $lt: 0,
              $gt: -3
        ...
        },
        {
          $project: {
            wallet: '$wallet.name',
            balanceSatoshis: 1,
            balance: { $divide: ['$balanceSatoshis', divider] },
            _id: 0,
            addresses: 1
          }
        },
