    ...
    const retryAddressTxCursor = async () => {
      ...
      while (await addressTxCursor.hasNext()) {
        ...
        if (addressTx) {
          ...
          // BCD HACK! this is a BAD solution. instead we should WRITE to database updated value, but DB is already synced
          // upd. now we dont want to write updated value
          if (addressCoin.coin.chain == 'BCD')
            addressCoin.coin.value = (addressCoin.coin.value || 0) * 1;
